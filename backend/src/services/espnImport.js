...
async function runFullImport(espnLeagueId, userId, db, cookies = {}, targetLeagueId, selectedSeasons) {
  ...
  try {
    ...
    if (mostRecentSeason) {
      detectedSettings = settingsMapper.mapSettingsForPlatform('espn', mostRecentSeason)

      if (!targetLeagueId && detectedSettings) {
        try {
          const updateData = { settings: { ...(clutchLeague.settings || {}), ...detectedSettings.settings, importedFrom: 'espn' } }

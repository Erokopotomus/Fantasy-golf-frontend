...
### Current Status: PHASE 5 — IN PROGRESS · Year 1 Decision Capture LIVE (May 16 2026)

**May 16 update — Decision Capture Year 1 end-to-end shipped.** Eight primitives instrumented in prod (draft pick, lineup decision, waiver/FA claim, trade event, naked roster drop, watchlist add/remove, auction nomination, shared reason chips). Migration 51 adds 5 model extensions + 2 new tables (WatchlistEvent, AuctionNomination). Backend write paths all snapshot state-at-decision-time. Frontend `<ReasonChipsPicker>` wired into trade modal + waiver claim. LINEUP_VIEWED PostHog event for paralysis detection. `leagueContextService.js` populates standings/weeks/playoffs on demand (roster strength + win prob + schedule strength deferred to A.2 — needs power-rank model). Bias engine = Year 2+ read layer; nothing surfaced to users in Year 1. Spec: `docs/CLUTCH_DECISION_CAPTURE_SPEC.md` v3.

**Same day — Railway landmine defused.** `railway.toml` was running `prisma db push --accept-data-loss && npm start`, silently sync'ing schema.prisma → prod before migrations and silently dropping columns on any rename. Removed db push. Migrations are sole schema authority. Memory: `project_clutch_deploy_model.md`.
...
**May 19 update — NFL career-data backfill (Phase 7 sub-task) COMPLETE.** 1999-2025 historical career data backfilled for 1,637 fantasy-relevant players (position-tiered top-N per season). 71,447 NflPlayerGame rows added in one shot. New `NflPlayerDataState` model (migration 55) + lazy-fetch fallback wired into `GET /api/nfl/players/:id`. PlayerDrawer click now opens on Prep depth chart + What Changed move rows for any active 2026 player — drill-down progressive disclosure. Verified ghost legends: Rodgers 21 seasons (2005-2025), Brady 23 (2000-2022), Manning 16 (1999-2015 — pre-nflverse floor at 1999), Stafford 17, Allen 8. Two latent bugs surfaced + fixed during the run: (1) `syncScheduleRaw` defined but never exported from `nflHistoricalSync.js`, (2) nflverse's new 2024+ stats schema dropped the `game_id` column — gameMap now keys by season+week+team (`runFilteredSeason`), (3) empty-string stat fields (notably `target_share` pre-2006) leaked into SQL as syntax errors — `numOrNull` helper normalizes. Spec + plan in `docs/plans/2026-05-19-nfl-career-backfill-{design,plan}.md`.

**Same day — FFC ADP sync wired as primary, not fallback.** Standalone `backend/src/services/nfl/ffcAdpSync.js` + runner script pulls Fantasy Football Calculator consensus ADP across PPR / Half PPR / Standard and `updateMany`s into existing `sleeper_consensus` `NflPlayerProjection` rows (no parallel-source rows). DST team-abbr fallback for "Denver Defense" → matches `nflTeamAbbr='DEN'`. 15s AbortController timeout per fetch (FFC API occasionally hangs). First successful run: PPR 170/170 matched, Half PPR 143/143, Standard 145/146 (Joshua Palmer trade to KC the only miss). Coverage now ~8% of total sleeper_consensus rows = 100% of the draftable FFC pool (the remaining 1900 rows are deep waiver/streaming players with no meaningful ADP anywhere).

**Same day — Mock Draft Prep-aesthetic rebuild COMPLETE.** 13-task plan shipped end-to-end via subagent-driven development (3-stage review per task: implementer + spec compliance + code quality, plus final cumulative review). Three surfaces (Lobby / Room / Recap) migrated to the editorial × Topps aesthetic + data swap to the projection-backed pool. New `frontend/src/components/lab-draft/` directory (7 primitives: `teamColorHelpers`, `LabDraftMasthead`, `FormatPill`, `FormatToggleGroup`, `PlayerRowAccent`, `MyTeamPanel`, `SortToggle`). New backend `GET /api/nfl/draft-players` endpoint joins `NflPlayerProjection` + `Player` + `NflTeam` with inline team colors, ADP-sorted draftable pool of ~170 (PPR). New canonical routes `/lab/mock-draft/*` with redirects from `/mock-draft/*`. PrepHub adds CTA tile linking to the lobby. **Hard rule honored:** 2887-line `MockDraftRoom.jsx` draft state machine (timer, snake/auction nominator, pick advancement, bid handling) byte-for-byte untouched — verified by off-limits identifier count (142 → 140, both removals cosmetic budget-display markup). Sport-neutral "Lab draft" aesthetic vocabulary established for future surfaces (NFL gets team-color overlays as a sport-specific layer; golf will get its own when DataGolf surfaces are migrated). Decisions captured: lobby-only scoring toggle (no mid-draft switching), ADP-default sort with hybrid Projected toggle, `/lab/mock-draft` as top-level Lab sibling (not under `PrepSectionNav`). Final review caught a deploy-blocker: `api.getDraftPlayers` collided with existing two-arg method used by live league drafts (`useDraft.js:124`) — renamed to `getNflDraftPlayers` before push. Spec: `docs/plans/2026-05-19-mock-draft-prep-rebuild-design.md`. Plan: `docs/plans/2026-05-19-mock-draft-prep-rebuild-plan.md`. Queue items #214 (SoS heatmap per position) + #215 (persist `scoring` on `MockDraftResult`) surfaced during build.

**May 18 update — NFL Data Spine + Prep Hub (Phase 5H) COMPLETE.** 18-task plan shipped end-to-end in one day. Backbone NFL offseason data layer (rosters, coaching staffs, unit ranks 3-yr, 2026 projections, end-of-season historical snapshots) plus 4-page user-facing surface in the Lab: `/lab/prep` overview, `/lab/prep/teams` (conference/division layout + GRID view), `/lab/prep/teams/:abbr` (team detail with inline "What changed for {abbr}"), `/lab/prep/changes` (league-wide division-grouped coaching board + position-filtered biggest moves split-pane + centered delta-bar viz for unit rank movement), `/lab/prep/quiz` (SM-2 spaced-repetition flashcards with team-color identity per card). Migration 54 adds 6 models. ≥300 active quiz cards generated. Unblocks Manager Intelligence `pick_overpay_rate` / `pick_reach_rate` extractors (NFL ADP data now in DB). Editorial × Topps annual aesthetic with 32 team-color washes throughout. Spec: `docs/plans/2026-05-18-nfl-data-spine-design.md`. Plan: `docs/plans/2026-05-18-nfl-data-spine-plan.md`.

---

### Phase 5 — IN PROGRESS (First Season Live)
> Phases 1-4 complete (4E partially complete). Phase 5B (Clutch Rating V2) built. Phase 5C (Prediction Expansion + Compact Table) complete. Phase 6 (AI Engine) complete — including Personal AI Coach Vault (per-user coaching memory). League Vault V2 + Commissioner blog built. ESPN historical backfill (2018-2026, 9 years) complete. DataGolf SG backfill (2018-2026) complete. Tournament intelligence + SG Intelligence (radar charts, trend viz) live. Golf Hub + Season Race + Compare page live. Live fantasy scoring pipeline + scoring page redesign live. Board editor overhauled (compare mode, SG columns, auction values, reason chips). Draft room season stats + tooltips. iPod Reframe (all 6 phases) complete. Profile enhancement (avatar upload, username, backend persist) complete. Leagues dropdown nav. **Friend Testing Sprint (Mar 2026):** New user flow fixed (avatar bug, join-by-code double-join, post-signup routing). Prove It page overhauled (Golf Slate default, AI coach engagement banner, top picks spotlight, PlayerDrawer integration). DraftRecap completely rewritten with cinematic reveal, SG radar charts, team SG power ranking, AI coach commentary, animated counters, interactive draft board. Branded HTML email templates (league invite + vault invite). Silent error capture system built (frontend service + ErrorBoundary + backend AppError model + admin endpoints + churn detection). Dead route redirects. ManagerProfile avatar + rating placeholder. Any-member invite permissions. **Draft-Day Readiness Audit (Mar 3):** Draft grading algorithm fixed (adpDiff was inverted — reaches showed as steals; value baseline raised so Scheffler #1 = A not B). Draft room mobile layout overhauled (board 30%/55% split, compact DraftHeader, compact DraftTimer, MobileNav hidden on draft routes). DraftDashboard tab PlayerDrawer click handlers added (was completely missing — no player clicks worked). All queued as items 072-074. **Post-Draft Engagement Loop (Batch C, Mar 3):** Designed and queued 7 items (075-081) covering: email↔notification service wiring, weekly tournament recap emails, prediction resolution notifications, live roster player alerts (hot round, leader, eagle, cut danger), draft recap emails, personalized coach briefing upgrade (real data instead of templates), frontend notification preferences UI. Architecture: Resend API for email, existing notificationService gains email channel, new `engagementAlerts.js` service for live push alerts, cron at Sun 11PM ET for weekly recaps. **Phase 5 Batch D (Mar 3 late):** Batch C (075-081) all DONE by Claude Code. Admin error dashboard (082) DONE. Manager profile Phase 5A enhancements — avatar upload, sports badges, recent calls (083) DONE. Queued items 084-089: performance charts + call type breakdown on profile (084), manager leaderboard page with filters/Hot Right Now/sort (085), achievement unlock engine with criteria evaluation + cron + event triggers (086), badge showcase + achievement progress bars on profile (087), nav links for leaderboard + profile quick access (088), verified badge + quick stats bar (089). **Phase 5C Prediction Expansion (Mar 3 late):** Items 090-091 DONE. Golf prediction categories expanded from 1 type (SG benchmark) to 8 types: tournament winner, top 5/10/20, make/miss cut, round 1 leader, H2H matchups, SG performance call reframe. Auto-resolution service (`golfPredictionResolver.js`) with Sun 10:30 PM + Mon 8 AM crons. **Prove It Compact Table Redesign (Mar 3):** Replaced pill-driven slate with horizontal table — all prediction types as columns, 12-15 players visible, one-tap cell cycling, sticky headers. ~194 lines net reduction. **Personal AI Coach Vault (Mar 3):** Per-user coaching memory system — 6 vault document types in PostgreSQL, Memory Writer cron (Wed 4:30 AM), Coach Context Assembly, Coach Settings page, 7 new API routes, 2 new Prisma models. **Mobile Visual Audit + Fixes (Mar 4):** Full visual mobile audit at 390px (iPhone 14) across 11 pages. Critical draft room bug found and fixed (player names invisible at 0px width — item 095). Landing page hero overflow fixed (096, overflow-hidden contains decorative blob). Touch targets all bumped to 44×44px minimum (098). Prove It FAB overlap fixed (099). Accessibility aria-labels added to nav icons (100). PageSpeed mobile audit completed (score 61, improvements queued as 097). Items 095-100 all DONE. Two minor polish items queued (101-102: sport filter emoji aria-labels, coach briefing link touch target). DNS migration from Netlify to Vercel initiated (A records pointed, CNAME for www added). Email invite 500 error fixed (093). Coach settings fetch error fixed (094). **Invite Flow Fix Sprint (Mar 4 late):** GirthBrooks09 missing from invite dropdown — root cause was if/else alias-only logic; fixed with parallel fetch + smart merge of OwnerAlias + HistoricalSeason sources (109, commits dd8e17a + 8705b9e). Join league link "League not found" — Express route ordering bug, `/:id` captured literal "preview-by-code" as param; fixed by moving routes above `/:id` (110, commit abc61f7). Prisma crash on preview-by-code — `sport` scalar field used in `include` (only relations allowed); latent bug exposed by route fix; removed from include (111, commit b395fcd). GirthBrooks09 missing from vault reveal — unmapped owners marked `isActive: false` by useVaultStats.js; fixed to treat unmapped owners as active (112, commit f67b505). All 4 fixes deployed and verified live. Josh (GirthBrooks09) successfully joined B Squad Bros. **First real draft completed: Mar 5 Wed.** **Draft Night + Live Tournament Sprint (Mar 5):** Arnold Palmer Invitational R1 live during draft. Stale 2025 tournament data cleaned from DB (items 129-132, 139-140). Premium glassmorphic tournament banner + LiveScoringWidget redesign deployed (items 143-151). ESPN hole-by-hole sync fixed — `espnClient.js` was using native `fetch()` which silently failed on Railway; converted to `axios` (item 152). 5th starter visibility fix (item 153). Stale LiveScore data guard added to `scoringService.js` — prevents previous-tournament data from bleeding into current roster display (Rasmus Hojgaard bug, item 155). Draft Complete banner auto-hide when tournament is live (item 156). Player scorecard LEFT drawer designed and queued (item 154). My Team page redesign with live scoring researched and queued (item 157). **Post-Draft Live Fixes Sprint (Mar 5-6):** BroMontana draft cost values fixed from spreadsheet truth — 12 years, 3-tier fuzzy name matching (item 158). My Team page completely redesigned — bigger headshots (56px), hero fantasy pts, glassmorphic scoreboard header, pill action bar, context-aware stats, front9/back9 scorecard grid, desktop sidebar with mini league leaderboard (items 159-160). Vault fixes: useVaultStats flattened history response (item 161), unmapped owner count inflation fixed (item 162). WeatherStrip added to LeagueHome sidebar (item 164) and GolfLiveScoring page (item 166). League scoring "No current NFL season" for golf leagues fixed — loading guard prevents race condition (item 165). LiveScoringWidget scorecard drawer fixes: stale selectedPlayer data fixed with freshPlayerData derivation + auto-refetch on THRU change (item 168). My Team live scoring overlay already handled in redesign (item 169). Round pills fix: always show R1-R4 with greyed-out unavailable rounds (was hidden when only 1 round had data). Weather fetch fix: useTournamentScoring now fetches weather for IN_PROGRESS tournaments (was UPCOMING only). **ESPN Hole Sync Root Cause Fix (Mar 6):** Diagnosed why hole data was stuck at 72 roundScores / 925 holeScores despite ESPN API returning full data. Root cause: batch transactions (`prisma.$transaction` chunks of 50) meant one bad record rolled back entire chunks and killed the sync. Rewrote `espnSync.js` with: inline RoundScore upserts (immediately available for hole refs), individual HoleScore upserts with per-hole try/catch, per-player try/catch so one bad player doesn't crash sync, detailed logging (unmatched names, rounds created/updated, hole errors). Added `POST /api/tournaments/:id/trigger-espn-sync` manual trigger endpoint. Manual sync triggered — data grew from 72→132+ roundScores, 925→1592+ holeScores. R2 data now flowing. Cron will keep updating every 5 min Thu-Sun. **Golf League Standings & Roster Enrichment Sprint (Mar 10-13):** League standings showed 0 points — GET /leagues/:id was reading Team.totalPoints (always 0, never updated). Fixed to compute real standings via calculateLeagueStandings() from Performance records for golf leagues. Also syncs Team.totalPoints in computeWeeklyResults. Added manual scoring trigger POST /api/sync/process-completed-weeks. Golf roto season-long leagues now show tournament progress ("1/32") instead of meaningless 0-0 W-L; H2H/NFL still show W-L. Backend returns seasonProgress { completed, total }. Roster player cards enriched with fantasy-relevant stats: season fantasy pts (current year only), avg pts/event, last 3 finishes (color-coded: gold top-5, green top-10, red MC), course history for upcoming tournament (starts, avg finish, best finish, missed cuts). Backend GET /teams/:id enriched with per-player fantasy stats computed from Performance records using league's scoring config. Course history uses RoundScore count to detect missed cuts (ESPN marks cut players as ACTIVE, so <=2 rounds = MC). Files: leagues.js (standings + progress), teams.js (roster enrichment), sync.js (manual trigger), fantasyTracker.js (totalPoints sync), LeagueHome.jsx (progress column), TeamRoster.jsx (enhanced cards).
...
- [x] **5G: Manager Intelligence (backward-looking characteristic extraction)** — COMPLETE (May 2026, migration 53). Backend pipeline + 3-page admin UI. 19 characteristics extracted from imported league history across 7 categories (pick_quality, positional, auction, trade, waiver, drop, outcome). Each row stored as `ManagerCharacteristic { confidenceScore, confidenceLabel: HIGH|MED|LOW, value, sampleSize, consistencyPct, effectSize, rawEvidence, sourceImportIds }`. Two new models: `ManagerCharacteristic`, `CharacteristicAggregate`. Extraction fires automatically on imports + vault claims + auto-match-on-join (fire-and-forget via `intelligence.runForUser`). Nightly 3 AM ET aggregate cron computes distribution snapshots for admin Library page. Architecture: `backend/src/services/intelligence/` (extractor.js orchestrator + 7 extractor files + lib helpers stats/seasonFetcher/draftPickParser/auctionParser/transactionParser). Admin UI at `/admin` Intelligence tab → Library (19 cards w/ histograms) → Detail (distribution + top users + threshold sliders + promote-to-coach toggle + admin notes) → User Profile (per-user characteristics grouped by category w/ expandable rawEvidence + recompute button). Layer 2 stub (MI-18) promotes characteristics into AI Coach Vault when admin flips `promoteToCoach=true` — facts land in `CoachingMemory.content.managerIntelligence.facts`. Backfill script at `backend/scripts/intelligence/backfill-all.js`. **League-relative principle:** comparison extractors (auction overpay, trade frequency, naked drop frequency) z-score against the user's own draft/league cohort, not generic platform baselines — captures whether you're the high-bidder in YOUR league vs being normal in a high-bid league. **Spec:** `docs/plans/2026-05-17-manager-intelligence-design.md` + `2026-05-17-manager-intelligence-plan.md`. **Closes queue #202** (Retroactive Rating Reveal data layer); user-facing reveal moment is now queue #208.

- [x] **5H: NFL Data Spine + Prep Hub (the Lab → /lab/prep)** — COMPLETE (May 2026, migration 54, plan `docs/plans/2026-05-18-nfl-data-spine-plan.md`). Backbone data layer + 4-page user-facing offseason prep surface, built so Eric can show up to NFL draft season knowing every roster, coaching change, and unit-rank movement cold. **Data layer:** 6 new Prisma models — `NflTeam` (32 franchises with conference/division/team colors), `NflCoachingStaff` (HC/OC/DC + position coaches, `hiredAt` nullable since GridironExperts scrape doesn't expose date), `NflRosterSlot` (current depth chart slots), `NflTeamUnitRank` (per-season unit ranks: OL / passing offense / rushing offense / passing defense / rushing defense — 3-year history 2023-2025), `NflPlayerProjection` (2026 projections + ADP from FantasyPros for MI extractor unblock), `PrepQuizCard` + `PrepQuizReview` + `PrepUserSettings` (SM-2 spaced repetition). **Sync services:** `nflRosterSync.js` (Sleeper API), `nflCoachingStaffSync.js` (curated + scraped), `nflUnitRankSync.js` (ESPN adjusted line yards + EPA/play), `nflProjectionSync.js` (FantasyPros). All registered as crons in `index.js`. **Historical snapshots:** `end_of_2024_season` + `end_of_2025_season` rosters backfilled from synced NflPlayerGame data (2,056 players in 2025 EOS) — drives the "What Changed" diff against current 2026 offseason rosters. **Quiz engine:** Template library + card generator produces ≥300 active flashcards across HC/OC/DC, starting QB, unit ranks, biggest movers. SM-2 scheduler is pure function (`sm2Scheduler.js`). Quiz session service balances new/due cards per user. **API routes:** `/api/prep/teams`, `/api/prep/teams/:abbr`, `/api/prep/changes`, `/api/prep/quiz/today`, `/api/prep/quiz/review`, `/api/prep/settings`. **Frontend (the Lab → Prep):** 4 pages — `/lab/prep` (hub overview with kickoff-to-NFL ticker, AFC/NFC/ALL tile grid, sample quiz card, biggest movers panel), `/lab/prep/teams` (conference/division layout + power-user GRID view), `/lab/prep/teams/:abbr` (team detail: hero strip, coaching card, 3-year unit trend, depth chart, inline "What changed for {abbr}" card), `/lab/prep/changes` (league-wide: division-grouped coaching board with NEW HC ring + badge, position-pill-filtered biggest moves split-pane with team-color-dominant destination treatment, centered delta bar viz for unit rank movement), `/lab/prep/quiz` (SM-2 4-button daily session with subject-team color identity per card, streak counter, focus mode + cards/day settings). Shared `PrepSectionNav` + `nflTeamColors.js` (32 franchise hex codes + hexToRgba). **Editorial × Topps annual aesthetic:** Dark slate broadcast ticker masthead, font-display Bricolage Grotesque, font-editorial Instrument Serif italic pull quotes, blaze accent lines, team-color washes on tiles + chips. **Lab nav:** "NFL Prep" entry added to Lab dropdown. **MI unblock:** `pick_overpay_rate` / `pick_reach_rate` extractors now have ADP data to compare against, league-relative.
...
- [ ] **NFL Fantasy Support** (Target: August 2026) — PARTIALLY COMPLETE
  ...
  - [x] NFL PlayerDrawer enrichment: position-specific stats (QB/RB/WR/TE/K/DST), bio strip, season selector (dynamic from synced data), enriched game log with W/L results
  - [x] NFL 2023 + 2025 historical sync extension (DS-6) — `nflHistoricalSync` now covers 2019-2025 by default
  - [x] **NFL career-data backfill (1999-2025)** — pool of 1,637 fantasy-relevant players (position-tiered top-N per season: 32 QB / 60 RB / 80 WR / 36 TE / 32 K) ingested with full weekly stat history. 71,447 NflPlayerGame rows added in one shot. Lazy-fetch fallback (5 most-recent seasons) on first drawer open for non-pool players. New `NflPlayerDataState` model (migration 55) tracks fetch state. Wired into `GET /api/nfl/players/:id`. Verified careers: Rodgers 21 seasons (2005-2025), Brady 23 (2000-2022), Manning 16 (1999-2015 — pre-1999 floor), Stafford 17, Allen 8. Spec: `docs/plans/2026-05-19-nfl-career-backfill-design.md`. Plan: `docs/plans/2026-05-19-nfl-career-backfill-plan.md`.
  - [x] PlayerDrawer click wired into Prep depth chart, "What changed for {abbr}" arrival/departure rows, and league-wide What Changed move rows (commit `8973116`) — first-tier progressive disclosure surface for NFL roster context.
...
Data Layer 1-7 done → Lab Phases 1-5 done → Phase 6 AI done → Import Intelligence done → Vault V2 done → Rating V2 done → Blog done → AI Coach reframe done → Tournament Intelligence done → ESPN Historical Backfill (2018-2026) done → DataGolf SG Backfill done → SG Intelligence done → Golf Hub + Season Race + Compare done → Live Scoring Pipeline done → Scoring Page Redesign done → Board Editor Overhaul done → iPod Reframe (6 phases) done → Profile Enhancement done → Nav + League UX done → Clutch Loop audit (19 fixes deployed) → Friend Testing Sprint (new user flow, DraftRecap overhaul, Prove It engagement, branded emails, error capture) done → Phase 5C Prediction Expansion + Compact Table done → Personal AI Coach Vault done → First real draft (Mar 5) done → Live tournament sprint done → Golf league standings + roster enrichment (Mar 10-13) done → Decision Capture Year 1 (May 16) done → Chopped Format NFL v1 (May 17) done → Phase 5H NFL Data Spine + Prep Hub (May 18) done → NFL career-data backfill 1999-2025 (May 19) done → FFC ADP sync wired as primary (May 19) done → **Mock Draft Prep-aesthetic rebuild (May 19) done — 7 new lab-draft primitives + 3 surface migrations + new draft-players endpoint, state machine untouched** → **Next: queue #214 (SoS heatmap per position), queue #215 (persist scoring on MockDraftResult), Phase 5 remaining (leaderboards 085, badges v2 086-087, profile charts 084, nav links 088, verified badge 089), NFL prediction categories, E2E browser test of new /lab/mock-draft flow**
...
*Last updated: May 19, 2026 (NFL career-data backfill 1999-2025 → FFC ADP sync wired as primary → Mock Draft Prep-aesthetic rebuild — 13 tasks via subagent-driven dev, draft state machine untouched, lab-draft primitives shipped, /lab/mock-draft canonical routes live)*
*Phases 1-4 complete (4E partially done). Phase 5B (Clutch Rating V2) complete. Phase 5C complete (prediction expansion + compact table redesign). Phase 6 complete (AI Engine + Coach reframe + Personal AI Coach Vault). Import Intelligence Pipeline complete. League Vault V2 complete. Commissioner blog complete. Brand System Wave 1 deployed. Tournament Intelligence & SG Intelligence complete. ESPN historical backfill (2018-2026) complete. DataGolf SG backfill complete. Golf Hub + Season Race + Compare page live. Live scoring pipeline + scoring page redesign live. Board editor overhauled + compare mode. iPod Reframe (6 phases) complete. Profile enhancement complete. Nav + League UX improvements live. Friend Testing Sprint complete. Batch C (engagement loop, 075-081) all DONE. Batch D (084-089) queued. Phase 5C (090-091) DONE. Admin error dashboard (082) DONE. Manager profile Phase 5A (083) DONE. Mobile visual audit (095-100) all DONE. DNS migration to Vercel complete. Premium glassmorphic tournament banner + LiveScoringWidget redesign (143-151) all DONE. ESPN hole sync fixed (152) DONE. 5th starter fix (153) DONE. Stale LiveScore guard (155) DONE. Draft Complete banner hide (156) DONE. BroMontana draft cost fix (158) DONE. My Team redesign (159-160) DONE. Vault fixes (161-162) DONE. WeatherStrip additions (164, 166) DONE. Golf scoring page fix (165) DONE. ESPN hole sync root cause fix (167, 170) DONE — batch transactions were killing data, rewritten with individual upserts + per-player error handling. Scorecard drawer fixes (168) DONE. **Golf League Standings & Roster Enrichment Sprint (Mar 10-13):** League standings 0-points bug fixed — GET /leagues/:id was reading Team.totalPoints (never updated); now computes real standings via calculateLeagueStandings() from Performance records for golf leagues, also syncs Team.totalPoints in computeWeeklyResults + manual trigger POST /api/sync/process-completed-weeks. Golf roto leagues show season progress ("1/32") instead of meaningless 0-0 W-L (H2H/NFL still show W-L). Roster player cards enriched with fantasy-relevant stats: season fantasy pts (current year), avg pts/event, last 3 finishes (color-coded: gold top-5, green top-10, red MC), course history for upcoming tournament (starts, avg finish, best, MCs). Backend GET /teams/:id enriched with per-player stats computed from Performance records using league scoring config; course history uses RoundScore count to detect missed cuts (ESPN marks cut players ACTIVE, <=2 rounds = MC). Player scorecard LEFT drawer (154) and My Team Phase 3+ queued. 630+ commits. 94+ database models. 178+ API endpoints. 71+ frontend pages. 38 cron jobs. 69 backend services. 53 migrations. 2 sports live.*

**All migrations (1-55) deployed to Railway.**

...
export default function ClutchRatingPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8 space-y-8">
        ...
        <div className="flex flex-col md:flex-row items-center gap-6">
          ...
          <div className="flex-1 text-center md:text-left">
            ...
            {isActive ? (
              ...
              <p className="text-sm text-text-primary/40 max-w-sm mx-auto md:mx-0">
                {activeCount > 0
      ...
      <section>
        <h2 className="text-lg font-display font-bold text-text-primary mb-1">Rating Components</h2>
        <p className="text-xs text-text-primary/30 mb-4">
      ...
      {bySport && bySport.length > 0 && (
        <section>
          <h2 className="text-lg font-display font-bold text-text-primary mb-1">Per-Sport Breakdown</h2>
          <p className="text-xs text-text-primary/30 mb-4">
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <h3 className="text-sm font-semibold text-text-primary">{sportName}</h3>
                  </div>
                  <div className="grid grid-cols-3 gap-2 text-center">
                    <div>
                      <p className="text-lg font-mono font-bold text-text-primary">{sp.wins || 0}</p>
                      <p className="text-[10px] text-text-primary/40">Wins</p>
                    </div>
                    <div>
                      <p className="text-lg font-mono font-bold text-accent-gold">{sp.championships || 0}</p>
                      <p className="text-[10px] text-text-primary/40">Titles</p>
                    </div>
                    <div>
                      <p className="text-lg font-mono font-bold text-text-primary">
                        {sp.winPct != null ? `${(sp.winPct * 100).toFixed(0)}%` : '—'}
                      </p>
                      <p className="text-[10px] text-text-primary/40">Win %</p>
                    </div>
                  </div>
                </div>
              )
      ...
      {/* ── 4. How to Improve ─────────────────────────────────────────────── */}
      <section>
        <h2 className="text-lg font-display font-bold text-text-primary mb-1">How to Improve</h2>
        <p className="text-xs text-text-primary/30 mb-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          ...
          >
            ...
            <h3 className="text-sm font-semibold text-text-primary group-hover:text-purple-300 transition-colors mb-1">
              The Lab
            </h3>
          ...
          >
            ...
            <h3 className="text-sm font-semibold text-text-primary group-hover:text-accent-gold transition-colors mb-1">
              Prove It
            </h3>
            <p className="text-[11px] text-text-primary/40 leading-relaxed">
      ...
      <section className="p-5 bg-dark-secondary/60 rounded-xl border border-[var(--card-border)]">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-semibold text-text-primary">How Do You Compare?</h3>
      ...
      <section className="p-5 bg-dark-secondary/60 rounded-xl border border-[var(--card-border)]">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-center">
          <div>
            <p className="text-[10px] uppercase tracking-wider text-text-primary/25 mb-1">Active Since</p>
            <p className="text-sm font-mono text-text-primary/60">
              {rating?.activeSince
          ...
          <div>
            <p className="text-[10px] uppercase tracking-wider text-text-primary/25 mb-1">Last Updated</p>
            <p className="text-sm font-mono text-text-primary/60">
              {rating?.updatedAt
          ...
          <div>
            <p className="text-[10px] uppercase tracking-wider text-text-primary/25 mb-1">Trend</p>
            <p className="text-sm text-text-primary/60 flex items-center justify-center gap-1">
              {rating?.trend ? (
                <>
      ...
      {user?.id && (
        <div className="text-center">
          <Link
            to={`/manager/${user.id}`}
            className="text-xs text-text-primary/30 hover:text-accent-gold transition-colors font-mono"
          >

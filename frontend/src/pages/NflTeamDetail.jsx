      ...
      <div className="bg-[var(--surface)] shadow-card border border-[var(--card-border)] rounded-xl p-6 mb-6">
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1">
            <h1 className="text-3xl font-display font-bold text-text-primary">{team.name}</h1>
            <div className="flex items-center gap-3 mt-1">
              <span className="text-text-primary/40 font-mono text-sm">{team.conference} {team.division}</span>
        ...
        {tabs.map(t => (
          <button
            ...
            className={`px-4 py-2 rounded-md text-sm font-bold capitalize transition-colors ${
              tab === t ? 'bg-gold/20 text-gold' : 'text-text-primary/40 hover:text-text-primary/60'
            }`}
          >
                ...
                <div className="bg-[var(--surface)] shadow-card border border-[var(--card-border)] rounded-xl overflow-hidden">
                  {players.map(p => (
                    ...
                    >
                      ...
                      <span className="text-text-primary font-medium flex-1">{p.name}</span>
                      {p.nflNumber && <span className="text-text-primary/20 font-mono text-sm">#{p.nflNumber}</span>}
                    </Link>
                  ))}
                </div>
              </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-[var(--card-border)]">
                ...
                <th className="text-left px-4 py-3 text-text-primary/40 text-xs font-mono uppercase">Venue</th>
              </tr>
            </thead>
            <tbody>
              {schedule.map(g => {
                ...
                return (
                  <tr key={g.id} className="border-b border-[var(--card-border)] hover:bg-[var(--surface-alt)]">
                    ...
                    <td className="px-4 py-3">
                      ...
                      <Link to={`/nfl/teams/${g.opponent}`} className="text-text-primary hover:text-gold text-sm">
                    ...
                    <td className="px-4 py-3 text-center">
                      {isFinal ? (
                        ...
                        <span className="text-text-primary/20 text-sm">{g.kickoff ? new Date(g.kickoff).toLocaleDateString() : 'TBD'}</span>
                      )}
                    </td>
                    <td className="px-4 py-3 text-text-primary/20 text-sm truncate max-w-[200px]">{g.venue || '-'}</td>
                  </tr>
                )
              })}
            </tbody>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-[var(--surface)] shadow-card border border-[var(--card-border)] rounded-xl p-6">
            ...
            <div className="grid lg:grid-cols-2 gap-6">
              <div className="space-y-3">
                <h4 className="text-text-primary/40 text-xs font-mono uppercase tracking-wider">Passing</h4>
                <div className="grid grid-cols-2 gap-x-6 gap-y-2 text-sm">
                  <div className="flex justify-between"><span className="text-text-primary/50">Yards</span><span className="text-text-primary font-mono">{stats.offense.passing.yards.toLocaleString()}</span></div>
                  <div className="flex justify-between"><span className="text-text-primary/50">YPG</span><span className="text-text-primary font-mono">{stats.offense.passing.ypg}</span></div>
                  <div className="flex justify-between"><span className="text-text-primary/50">TDs</span><span className="text-text-primary font-mono">{stats.offense.passing.tds}</span></div>
                  <div className="flex justify-between"><span className="text-text-primary/50">INTs</span><span className="text-text-primary font-mono">{stats.offense.passing.interceptions}</span></div>
                  <div className="flex justify-between"><span className="text-text-primary/50">Comp/Att</span><span className="text-text-primary font-mono">{stats.offense.passing.completions}/{stats.offense.passing.attempts}</span></div>
                  <div className="flex justify-between"><span className="text-text-primary/50">Comp %</span><span className="text-text-primary font-mono">{stats.offense.passing.attempts ? ((stats.offense.passing.completions / stats.offense.passing.attempts) * 100).toFixed(1) : 0}%</span></div>
                </div>
              </div>
              <div className="space-y-3">
                <h4 className="text-text-primary/40 text-xs font-mono uppercase tracking-wider">Rushing</h4>
                <div className="grid grid-cols-2 gap-x-6 gap-y-2 text-sm">
                  <div className="flex justify-between"><span className="text-text-primary/50">Yards</span><span className="text-text-primary font-mono">{stats.offense.rushing.yards.toLocaleString()}</span></div>
                  <div className="flex justify-between"><span className="text-text-primary/50">YPG</span><span className="text-text-primary font-mono">{stats.offense.rushing.ypg}</span></div>
                  <div className="flex justify-between"><span className="text-text-primary/50">TDs</span><span className="text-text-primary font-mono">{stats.offense.rushing.tds}</span></div>
                  <div className="flex justify-between"><span className="text-text-primary/50">Attempts</span><span className="text-text-primary font-mono">{stats.offense.rushing.attempts.toLocaleString()}</span></div>
                  <div className="flex justify-between"><span className="text-text-primary/50">YPC</span><span className="text-text-primary font-mono">{stats.offense.rushing.attempts ? (stats.offense.rushing.yards / stats.offense.rushing.attempts).toFixed(1) : 0}</span></div>
                </div>
              </div>
            </div>
            {stats.offense.epa && (
              <div className="mt-4 pt-4 border-t border-[var(--card-border)]">
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm">
                  ...
                  <div><span className="text-text-primary/40 block text-xs mb-1">EPA/Game</span><span className="text-text-primary font-mono font-bold">{stats.offense.epa.perGame}</span></div>
                  <div><span className="text-text-primary/40 block text-xs mb-1">Points/Game</span><span className="text-text-primary font-mono font-bold">{stats.offense.ppg}</span></div>
                </div>
              </div>
            )}
          </div>
          ...
          <div className="bg-[var(--surface)] shadow-card border border-[var(--card-border)] rounded-xl p-6">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm">
              ...
              <div><span className="text-text-primary/40 block text-xs mb-1">Sacks</span><span className="text-text-primary font-mono font-bold">{stats.defense.sacks}</span></div>
              <div><span className="text-text-primary/40 block text-xs mb-1">Interceptions</span><span className="text-text-primary font-mono font-bold">{stats.defense.interceptions}</span></div>
              <div><span className="text-text-primary/40 block text-xs mb-1">Defensive TDs</span><span className="text-text-primary font-mono font-bold">{stats.defense.tds}</span></div>
              <div><span className="text-text-primary/40 block text-xs mb-1">Fumbles Forced</span><span className="text-text-primary font-mono font-bold">{stats.defense.fumblesForced}</span></div>
              <div><span className="text-text-primary/40 block text-xs mb-1">Fumbles Recovered</span><span className="text-text-primary font-mono font-bold">{stats.defense.fumblesRecovered}</span></div>
              <div><span className="text-text-primary/40 block text-xs mb-1">Passes Defended</span><span className="text-text-primary font-mono font-bold">{stats.defense.passesDefended}</span></div>
              <div><span className="text-text-primary/40 block text-xs mb-1">Total Tackles</span><span className="text-text-primary font-mono font-bold">{stats.defense.tackles.total.toLocaleString()}</span></div>
            </div>
          </div>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-[var(--card-border)]">
                    ...
                    <th className="text-center px-4 py-2 text-text-primary/40 text-xs font-mono uppercase">GP</th>
                    <th className="text-right px-4 py-2 text-text-primary/40 text-xs font-mono uppercase">Total</th>
                    <th className="text-right px-4 py-2 text-text-primary/40 text-xs font-mono uppercase">PPG</th>
                  </tr>
                </thead>
                <tbody>
                  {stats.topPlayers.map((p, i) => (
                    <tr key={p.id} className="border-b border-[var(--card-border)] hover:bg-[var(--surface-alt)]">
                      ...
                      <td className="px-4 py-2.5 text-center text-text-primary/40 text-xs font-mono">{p.position}</td>
                      <td className="px-4 py-2.5 text-center text-text-primary/40 text-sm font-mono">{p.games}</td>
                      <td className="px-4 py-2.5 text-right text-gold font-mono font-bold text-sm">{p.fantasyPts}</td>
                      <td className="px-4 py-2.5 text-right text-text-primary/60 font-mono text-sm">{p.ptsPerGame}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <p className="text-center text-text-primary/20 text-xs font-mono">Data via nflverse | {stats.season} season</p>
        </div>
      )}

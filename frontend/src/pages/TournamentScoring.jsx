...
const PlayerOddsCard = ({ player, onClose }) => {
  ...
  return (
    <div className="rounded-xl border border-field-bright/30 bg-[var(--surface)] shadow-card overflow-hidden animate-fade-in">
      {/* Header */}
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 text-[11px] text-text-muted">
              ...
              <span>|</span>
              <span className={`font-semibold ${parseInt(player.score) < 0 ? 'text-field' : parseInt(player.score) > 0 ? 'text-live-red' : 'text-text-primary'}`}>
      ...
      {hasProbabilities ? (
        <div className="px-4 py-3 space-y-2.5">
          ...
          <ProbBar label="Win" value={probs.win} color="bg-crown" />
      ...
      <div className="grid grid-cols-3 gap-px bg-[var(--card-border)] border-t border-[var(--card-border)]">
        ...
        <div className="bg-[var(--surface)] px-3 py-2 text-center">
          <p className="text-xs font-semibold text-field">{player.birdies || 0}</p>
          <p className="text-[10px] text-text-muted">Birdies</p>
        </div>
        <div className="bg-[var(--surface)] px-3 py-2 text-center">
          <p className="text-xs font-semibold text-live-red">{player.bogeys || 0}</p>
...
const TournamentScoring = () => {
  ...
  const { tournament, leaderboard, weather, isLive, myPlayers, loading, error, refetch } = useTournamentScoring(tournamentId, leagueId)
  ...
  // UPCOMING — show full Tournament Preview intelligence page
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between px-1">
        <div className="flex items-center gap-3">
          ...
          {isLive && (
            <span className="text-xs text-text-muted flex items-center gap-1.5">
              <svg className="w-3.5 h-3.5 animate-spin text-field" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <button
          onClick={refetch}
          className="text-xs text-field hover:text-emerald-300 transition-colors font-medium"
        >
          Refresh
        </button>
      </div>
            ...
            <div className="flex items-center justify-between px-4 py-3 bg-gradient-to-r from-emerald-900/20 to-[var(--surface)] border-b border-[var(--card-border)]">
              <div className="flex items-center gap-2 min-w-0">
                ...
                {leagues && leagues.length > 0 && (
                  <select
                    ...
                    className="bg-[var(--bg-alt)] border border-[var(--card-border)] rounded-lg text-xs text-text-secondary px-2 py-1 focus:outline-none focus:border-field-bright/50 cursor-pointer min-w-0 truncate"
                  >
              ...
              {leagueId && (
                <div className="text-right flex-shrink-0">
                  ...
                  <p className="text-xl font-bold font-display text-field leading-tight">

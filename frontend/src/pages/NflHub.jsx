...
const quickLinks = [
  {
    label: 'Players',
    href: '/nfl/players',
    sublabel: 'Stats, rankings & profiles',
    icon: (
  ...
  },
  {
    label: 'Schedule',
    href: '/nfl/schedule',
    sublabel: '18 weeks · live results',
    icon: (
      <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-3 mb-10">
            {quickLinks.map(link => (
              <Link
                key={link.href}
                to={link.href}
                className="bg-[var(--surface)] border border-[var(--card-border)] rounded-xl p-4 flex flex-col items-center gap-2 hover:bg-[var(--surface-alt)] hover:border-orange/30 transition-all group shadow-card text-center"
              >
                ...
                {link.sublabel && (
                  <span className="text-text-muted text-[10px] leading-tight">{link.sublabel}</span>
                )}
              </Link>
            ))}
          </div>

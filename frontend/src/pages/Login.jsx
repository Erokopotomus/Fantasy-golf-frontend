      ...
      <div className="flex-1 flex items-center justify-center px-4 sm:px-6 lg:px-8 py-12">
        <div className="max-w-md w-full">
          ...
          {poolContext ? (
            <>
              ...
              <h1 className="text-2xl sm:text-3xl font-bold font-display text-text-primary mb-2 leading-tight">
                Sign in to enter <span className="text-blaze">{poolContext.name}</span>
              </h1>
              <p className="text-text-secondary mb-8 leading-relaxed">
                {poolContext.tournament ? `${poolContext.tournament} pool. ` : ''}We'll take you right back to your picks once you're in.
              </p>
            </>
          ) : (
            <>
          ...
          <p className="mt-8 text-center text-text-secondary">
            ...
            <Link to={redirectParam ? `/signup?redirect=${encodeURIComponent(redirectParam)}` : '/signup'} state={{ from: location.state?.from }} className="text-gold hover:text-gold/80 font-medium transition-colors duration-300">
              Sign up free
            </Link>
          </p>
        </div>

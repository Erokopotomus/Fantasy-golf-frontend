...
const BuyBackModal = ({ isOpen, onClose, onConfirm, loading }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
      <div className="bg-[var(--surface)] rounded-xl p-6 max-w-md w-full border border-crown/50">
        {/* Warning icon */}
        <div className="w-16 h-16 rounded-full bg-crown/20 flex items-center justify-center mx-auto mb-4">
        ...
        {!confirmed ? (
          <>
            <p className="text-text-secondary text-center mb-6">
              You only get <span className="text-crown font-semibold">one buy-back</span> per season. Are you sure you want to use it now?
            </p>
          ...
          <p className="text-text-secondary text-center mb-6">
            ...
            <span className="block text-crown font-semibold mt-2">This action cannot be undone.</span>
          </p>
        )}
        ...
        <div className="flex gap-3">
          <button
            onClick={onClose}
            disabled={loading}
            className="flex-1 py-2 bg-[var(--card-bg)] text-text-primary font-medium rounded-lg hover:bg-[var(--bg)] transition-colors disabled:opacity-50"
          >
            Cancel
          </button>
          <button
            ...
            className={`flex-1 py-2 font-semibold rounded-lg transition-colors disabled:opacity-50 ${
              confirmed
                ...
                : 'bg-crown text-slate hover:bg-crown'
            }`}
          >

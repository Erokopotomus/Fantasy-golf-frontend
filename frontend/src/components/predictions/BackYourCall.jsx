  ...
  if (!expanded) {
    return (
      <button
        ...
        className="w-full mt-2 py-1.5 text-[11px] text-white/30 hover:text-white/50 transition-colors flex items-center justify-center gap-1.5"
      >
    ...
    <div className="mt-2 pt-2 border-t border-white/5 space-y-2.5">
      ...
      <div>
        <textarea
          ...
          rows={2}
          className="w-full bg-white/[0.04] border border-white/[0.08] rounded-lg px-3 py-2 text-xs text-white/80 placeholder-white/20 focus:outline-none focus:border-gold/30 resize-none"
        />
        <div className="text-right text-[9px] text-white/15 mt-0.5">{thesis.length}/280</div>
      </div>
            ...
            <button
              ...
              className={`flex-1 py-1 rounded text-[10px] font-medium transition-colors ${
                confidenceLevel === level
                  ? level <= 2 ? 'bg-white/15 text-white/70'
      ...
      {!compact && (
        <div>
          <div className="text-[10px] text-white/30 mb-1.5">Key factors</div>
              ...
              <button
                ...
                className={`px-2 py-0.5 rounded-full text-[10px] transition-colors ${
                  selectedFactors.includes(factor)
                    ...
                    : 'bg-white/[0.04] text-white/25 border border-transparent hover:text-white/40'
                }`}
              >

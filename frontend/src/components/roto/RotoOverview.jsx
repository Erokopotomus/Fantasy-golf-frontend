...
const RotoOverview = ({ team, categories, categoryLabels, totalTeams, currentUserId }) => {
  ...
  return (
    <Card className={isUser ? 'ring-2 ring-accent-green/30' : ''}>
      {/* Header */}
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          <div className={`w-12 h-12 rounded-full flex items-center justify-center text-lg font-bold ${
            isUser ? 'bg-accent-green/20 text-accent-green' : 'bg-dark-tertiary text-text-secondary'
          }`}>
            {team.avatar}
          </div>
          <div>
            <h3 className={`text-lg font-semibold ${isUser ? 'text-accent-green' : 'text-white'}`}>
              {team.name}
        ...
        <div className="text-right">
          <p className="text-3xl font-bold text-accent-green">{team.totalRotoPoints}</p>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-6">
        {categories.map(cat => {
          ...
          if (pct <= 0.33) bgColor = 'bg-accent-green/10 border-accent-green/30'
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          <div className="space-y-2">
            {strengths.map(cat => (
              <div key={cat.id} className="flex items-center justify-between p-2 bg-accent-green/10 rounded">
                <span className="text-sm text-white">{cat.label}</span>
                <span className="text-sm font-semibold text-accent-green">#{cat.rank}</span>
              </div>
            ))}
          </div>
        </div>

...
/**
 * Composite power score: when CPI is available, blend CPI (55%) + SG Total (45%).
 * Otherwise, use SG Total as the sole signal.
 * CPI range ~[-3,+3], SG Total ~[-2,+3].  Normalise both to 0-100.
 */
const computePowerScore = (p) => {
  const sg  = p.sgTotal ?? 0 // roughly -2 to +3
  const sgNorm = Math.min(100, Math.max(0, ((sg + 2) / 5) * 100))
  if (p.cpi != null) {
    const cpiNorm = Math.min(100, Math.max(0, ((p.cpi + 3) / 6) * 100))
...
/**
 * Lineup Optimizer — ranks roster players by power score and suggests
 * the best active lineup.  Uses CPI + SG Total composite when CPI is
 * available, otherwise SG Total alone.  Zero API calls — works entirely
 * from prop data passed by the parent.
 */
const LineupOptimizer = ({ roster, maxActive, currentActiveIds, onApply, onClose }) => {
  ...
  // Check if any player has CPI data (for sort option visibility)
  const hasCpi = useMemo(
    () => roster.some(p => p.cpi != null),
    [roster]
  )

  // Best ranking function: composite power score from props
  const getRankScore = (p) => computePowerScore(p)
  ...
  return (
    <div className="bg-[var(--surface)] border border-[var(--card-border)] rounded-xl overflow-hidden">
      ...
      <div className="px-3 pb-3">
        <div className="grid grid-cols-[auto_1fr_50px_50px_50px] gap-x-2 text-[10px] text-text-muted uppercase tracking-wider font-medium px-2 pb-1">
        ...
        <div className="space-y-1">
          {sorted.map((p, idx) => {
            ...
            return (
              <div
                key={p.id}
                className={`grid grid-cols-[auto_1fr_50px_50px_50px] gap-x-2 items-center p-2 rounded-lg transition-colors ${
                  isOptimal
      ...
      {hasChanges && onApply && (
        <div className="px-3 pb-3">
          ...
          <p className="text-[10px] text-text-muted text-center mt-1.5">
            This will set the top {maxActive} players by power score as active
          </p>
        </div>
      )}

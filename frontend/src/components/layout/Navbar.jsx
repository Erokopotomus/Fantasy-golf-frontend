...
const navDropdownItems = {
  ...
  lab: {
    ...
    items: [
      ...
      { label: 'NFL Prep', to: '/lab/prep', icon: <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2M12 11h.01M12 15h.01M16 11h.01M16 15h.01M8 11h.01M8 15h.01" /></svg> },
...
const accentColors = {
  emerald: { icon: 'text-field', hover: 'hover:text-field', active: 'text-field', border: 'border-field-bright/20', hubBg: 'bg-field-bright/10', hubText: 'text-field', hubIcon: 'text-field' },
...
const leagueSportConfig = {
  GOLF: { label: 'Golf', dot: 'bg-field', text: 'text-field' },
  NFL:  { label: 'NFL',  dot: 'bg-blaze',  text: 'text-blaze' },
}
    ...
    <div ref={ref} className="relative" onMouseEnter={handleEnter} onMouseLeave={handleLeave}>
      ...
      {open && (
        <div className="absolute left-0 top-full mt-1 w-56 bg-[var(--nav-bg)] border border-white/10 border-gold/20 rounded-xl shadow-lg z-50 overflow-hidden">
          ...
          {leaguesLoading ? (
            <div className="px-3.5 py-3 flex items-center gap-2"><div className="w-3 h-3 border-2 border-gold/30 border-t-gold rounded-full animate-spin" /><span className="text-xs text-white/40">Loading...</span></div>
          ) : !hasLeagues ? (
                  ...
                  {profileMenuOpen && (
                      <div className="absolute right-0 mt-2 w-52 bg-[var(--nav-bg)] border border-white/10 rounded-card shadow-lg z-20 py-1">
                        ...
                        <button
                          ...
                          className="block w-full text-left px-4 py-2 text-sm text-live-red hover:bg-surface-hover transition-colors"
                        >
                          <div className="flex items-center gap-2">
                            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
